...
from tvsp2xmltv import defaults, logger, tvsGrabber
...
""" % (
    ...
    try:
        ...
        parser.add_argument(
            ...
            dest="time",
            default="00:00",
            help="The time for the control file",
        )
        parser.add_argument(
            "-d",
            "--days",
            dest="days",
            default="14",
            help="numberof days for the control file",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            dest="option",
            help="options from xmltv2vdr call [default: %(default)s]",
            metavar="option",
            nargs="*",
        )
        parser.add_argument(
            ...
            "--checkchannels",
            action="store_true",
            help='Go to http://www.vdr-wiki.de/wiki/index.php/Xmltv2vdr-plugin and safe the "Verbindliche EPG-Senderliste" to an text file called "channelids.txt"',
        )
        parser.add_argument(
            "-m",
            "--checkchannelmap",
            action="store_true",
            help="Show the diff between channel map and server.",
        )
        ...
        logger.log(
            "{0}:\n{1}\n{2}".format(program_name, repr(e), traceback.format_exc()),
            logger.ERROR,
        )
        return 2
