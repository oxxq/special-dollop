...
def m3u8():
    ...
    req = Request(
        "https://www2.vavoo.to/live2/index?output=json",
        headers={
    ...
    for c in channel:
        ...
        if not test:
            name = re.sub(
                "( (AUSTRIA|AT|HEVC|RAW|SD|HD|FHD|UHD|H265|GERMANY|DEUTSCHLAND|1080|DE|S-ANHALT|SACHSEN|MATCH TIME))|(\\+)|( \\(BACKUP\\))|\\(BACKUP\\)|( \\([\\w ]+\\))|\\([\\d+]\\)",
                "",
                c["name"].encode("ascii", "ignore").decode("ascii"),
            )
            logo = c["logo"]
    ...
    for c in channel:
        ...
        if row:
            ...
            if not str(row[3]) == "" and not str(row[4]) == "":
                tf.write(
                    '\n#EXTINF:-1 tvg-name="%s" group-title="%s" tvg-logo="%s" tvg-id="%s",%s'
                    % (row[1], row[2], row[3], row[4], row[6])
                )
            elif not str(row[3]) == "" and str(row[4]) == "":
                tf.write(
                    '\n#EXTINF:-1 tvg-name="%s" group-title="%s" tvg-logo="%s",%s'
                    % (row[1], row[2], row[3], row[6])
                )
            elif not str(row[4]) == "" and str(row[3]) == "":
                tf.write(
                    '\n#EXTINF:-1 tvg-name="%s" group-title="%s" tvg-id="%s",%s'
                    % (row[1], row[2], row[4], row[6])
                )
            else:
                tf.write(
                    '\n#EXTINF:-1 tvg-name="%s" group-title="%s",%s'
                    % (row[1], row[2], row[6])
                )
...
def sig():
    if not os.path.exists(json_file):
        veclist = requests.get(
            "https://raw.githubusercontent.com/michaz1988/michaz1988.github.io/master/data.json"
        ).json()
    ...
    try:
        if "vecs" in vecs:
            ...
            while not sig and i < 50:
                ...
                req = requests.post(
                    "https://www.vavoo.tv/api/box/ping2", data=vec
                ).json()
                if req.get("signed"):
                    sig = req["signed"]
