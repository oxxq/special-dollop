...
from . import defaults, logger, model, pictureLoader
...
class TvsGrabber(object):
    ...
    def _get_channel(self, date, tvsp_id):
        ...
        url = "https://live.tvspielfilm.de/static/broadcast/list/{0}/{1}".format(
            tvsp_id, date
        )
        print(url)
        ...
        if r.status_code == requests.codes.ok:
            ...
            logger.log(
                "{0} returned status code {1}".format(r.url, r.status_code),
                logger.WARNING,
            )
        ...
        for chan_id in self.channel_list:
            ...
            if not tvsp_id:
                if chan_id in defaults.combination_channels:
                    ...
                    logger.log(
                        "Channel {0} not in channel map.".format(chan_id),
                        logger.WARNING,
                    )
                    continue
    ...
    def __grab_day(self, date, tvsp_id, channel_id):
        ...
        for s in data:
            ...
            try:
                ...
                logger.log(
                    "Failed to fetch Channel {0} at {1}".format(tvsp_id, date),
                    logger.WARNING,
                )
