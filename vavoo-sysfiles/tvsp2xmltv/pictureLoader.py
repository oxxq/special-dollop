...
import glob
from io import open
from os import fchmod, path, remove
...
def cleanup_images():
    if defaults.remove_orphaned_images:
        files_in_dir = glob.glob(
            path.abspath(path.join(defaults.epgimages_dir, "*.jpg"))
        )
        ...
        if self.programme.images:
            if len(self.programme.images) > 0:
                ...
                for im in sorted(self.programme.images):
                    i += 1
                    f = self.__download_image(
                        im["size{0}".format(defaults.size_of_images)],
                        defaults.epgimages_dir,
                    )
                    if f:
        ...
        if path.exists(full_file_path):
            ...
            if file_suffix in suffix_list and i.status_code == requests.codes.ok:
                with open(full_file_path, "wb") as f:
                    ...
                    f.write(i.content)
                    logger.log(
                        "new file downloaded: {0}".format(full_file_path), logger.DEBUG
                    )
            else:
                return False
