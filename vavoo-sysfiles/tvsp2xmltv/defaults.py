...
thumb_id_map = {"DOWN": 1, "MIDDLE": 2, "UP": 3}

combination_channels = {"nickcomedy.de": ["NICK", "CC"]}
...
def write_controlfile(grab_time, grab_days):
    print(
        "Writing Controlfile [{0}, {1}, {2}]".format(control_file, grab_time, grab_days)
    )
...
def checkchannelmap():
    ...
    print(
        "The following channels included in the channel map are NOT provided by the server:"
    )
    for name, val in channel_map.items():
