...
if [ "$1" == "epg" ] || [ "$1" == "" ]; then
    ...
    if ! [ -e "$xmltv" ]; then
        xmltv="$HOME/.local/bin/tvspielfilm2xmltv";
        if ! [ -e "$xmltv" ]; then
            xmltv="/usr/lib/python2.7/site-packages/tvspielfilm2xmltv.py";
            if ! [ -e "$xmltv" ]; then echo "Error! tvspielfilm2xmltv not found!"; exit 1; fi;
        fi;
    fi;
    ini="tvspielfilm2xmltv.ini";
    if ! [ -e "$ini" ]; then echo "Error! tvspielfilm2xmltv.ini not found!"; exit 1; fi;
    con="tvspielfilm2xmltv.control";
...
"SUPER RTL" "SWR/SR" "Syfy" "tagesschau24" "Tele 5" "TLC" "TNT Comedy" "TNT Film" "TNT Serie" "TOGGO plus" "Universal Channel HD" "VOX" "VOXup" "WDR" "ZDF" "ZDFinfo" "ZDFneo";
    ...
    gzip $epgfile;
    if [ -e "${epgfile}.gz" ]; then
        echo "epg.xml.gz successful created @ $(dirname $epgfile)";
        exit 0;
